...
import { AlignLeft, Search, Trash2, TrendingUp, User } from 'lucide-react'
...
const DragManager: React.FC<{
  ...
  useFrame(() => {
    ...
    smoothedDeltaRef.current.x = THREE.MathUtils.lerp(
      smoothedDeltaRef.current.x,
      rawDeltaX,
      smoothing,
    )
    smoothedDeltaRef.current.y = THREE.MathUtils.lerp(
      smoothedDeltaRef.current.y,
      rawDeltaY,
      smoothing,
    )
    ...
    const isOverShelf = targetX < -(SHELF_GAP / 2) && targetX > -(BOOKCASE_WIDTH + SHELF_GAP / 2)
    ...
    const distToTrash = Math.sqrt(
      (targetX - TRASH_POSITION[0]) ** 2 + (targetY - TRASH_POSITION[1]) ** 2,
    )
...
const CameraController: React.FC<{ targetBook: any; isZooming: boolean }> = ({
  targetBook,
  isZooming,
}) => {
  useFrame((state) => {
...
const TrashBin: React.FC<{ isHovered: boolean; position: [number, number, number] }> = ({
  isHovered,
  position,
}) => {
  return (
    <group position={position}>
      ...
      <Html position={[0, 1.2, 0]} center>
        <div
          className={`transition-[opacity,transform] duration-500 ${isHovered ? 'opacity-100 scale-110' : 'opacity-40 scale-100'}`}
        >
          <div className="flex flex-col items-center gap-1.5 pointer-events-none">
            <div
              className={`p-2.5 rounded-full backdrop-blur-md border ${isHovered ? 'bg-red-500/20 border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-white/5 border-white/10'}`}
            >
              <Trash2
                className={`w-5 h-5 ${isHovered ? 'text-red-400 animate-bounce' : 'text-white/40'}`}
              />
            </div>
            <span
              className={`text-[10px] font-bold tracking-[0.2em] uppercase drop-shadow-2xl ${isHovered ? 'text-red-400' : 'text-white/20'}`}
            >
              Discard Volume
            </span>
          </div>
...
const ThreeDLibraryPage: React.FC = () => {
  ...
  const [flyingBooks, setFlyingBooks] = useState<
    Record<string, { destination: [number, number, number]; onComplete: () => void }>
  >({})
  ...
  const getBookStatus = useCallback(
    (book: any): BookStatus => {
      ...
      if (
        queue.some(
          (t) =>
            t.gutenbergId === gutenbergId && (t.status === 'downloading' || t.status === 'queued'),
        )
      ) {
        return 'downloading'
      }
  ...
  const handleDownloadBook = useCallback(async () => {
    if (focusedBook) {
      ...
      enqueue({
        ...
        authors: focusedBook.authors
          ? authorsString(focusedBook)
          : (focusedBook.author ?? 'Unknown'),
  ...
  const handleDragUpdate = useCallback(
    (offset: [number, number, number], isOverShelf: boolean, isOverTrash: boolean) => {
    ...
    async (book: any) => {
      ...
      const moveDistance = Math.sqrt(
        dragOffset[0] * dragOffset[0] +
          dragOffset[1] * dragOffset[1] +
          dragOffset[2] * dragOffset[2],
      )
      ...
      if (isOverTrash && book._isLocal) {
        ...
        const trashDestination: [number, number, number] = [
          trashDestinationX,
          trashDestinationY,
          trashDestinationZ,
        ]
  ...
  return (
    ...
    >
          ...
          <group position={[-bookcaseX, 0, 0]}>
            ...
            {isOverDropZone && (
              <mesh position={[0, BOOKCASE_HEIGHT / 2, 0]}>
                <boxGeometry
                  args={[BOOKCASE_WIDTH + 0.2, BOOKCASE_HEIGHT + 0.2, SHELF_DEPTH + 0.1]}
                />
      ...
      {/* UI Overlay */}
      <div
        className={`absolute inset-0 pointer-events-none transition-all duration-1000 ${isZooming ? 'opacity-0' : 'opacity-100'}`}
      >
          ...
          <div className="bg-stone-950/90 backdrop-blur-2xl px-10 py-7 rounded-3xl border border-white/5 shadow-2xl animate-in fade-in zoom-in duration-500">
            <div className="flex flex-col items-center gap-6">
              ...
              <div className="text-center">
                ...
                <p className="text-white text-xl font-serif font-medium max-w-[300px] line-clamp-1">
                  {selectedBook.title}
                </p>
              </div>
            </div>
