...
- [ ] Task: Create a `Bookcase` component using box geometries with high-quality wood textures/materials.
- [ ] Task: Implement a `BookMesh` component that accepts a cover texture and generates a realistic book shape.
- [ ] Task: Create a layout engine to position `BookMesh` components on the `Bookcase` shelves based on library data.
- [ ] Task: Implement Orbit Controls for navigation.
...
- [ ] Task: Add hover interactions to `BookMesh` (e.g., slight protrusion, highlight).
- [ ] Task: Configure high-quality shadows and contact shadows for the bookshelf.
- [ ] Task: Implement the "Cinematic Zoom" camera animation using `framer-motion-3d` or `GSAP`.
- [ ] Task: Integrate the transition logic to navigate to the 2D reader after the zoom completes.
...
- [ ] Task: Fetch book data from SQLite/Rust backend and map to 3D shelf positions.
- [ ] Task: Implement texture loading and caching for book covers.
