...
- [ ] Task: Verify the standalone binary runs correctly on the current platform (macOS) without the virtual environment.
- [ ] Task: Update `tauri.conf.json` to include the sidecar binary and configure the required naming convention (e.g., `qwen-tts-aarch64-apple-darwin`).
